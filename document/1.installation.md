...
オープンソースのライブラリを利用するときに、 .js ファイルをダウンロードして自分のディレクトリにコピーするような作業は必要なく、npm のサーバーに多くのライブラリが登録されており、`npm install`にようなコマンドでダウンロードやアンインストール、アップデートなどができます。
...
1. 実行時に必要なライブラリをインストールします。下記コマンドを実行します。
    ...
    * 各ライブラリの概要:
        ...
        * styled-components: react で CSS を管理するためのライブラリ
